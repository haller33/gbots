#!/usr/bin/env lua
-- bot_nars_goal_udp_client_only.lua
-- Galcon bot that connects to an externally started NARS/UDPNAR process.
-- No process management – assumes NARS is already listening on UDP_PORT.

local socket = require("socket")

-- ============================================================
-- CONFIGURATION – adjust these to match your running NARS
-- ============================================================
...
-----------------------------------------------------------------------
local function clean_narsese(line)
    -- Remove double dots before space
    -- line = line:gsub("%.%.", " . ")
    -- Ensure exactly one dot before space
    -- line = line:gsub("%. :", " . :")
    return line
end

local function send_to_nars(line)
  line_clean = clean_narsese(line)
  if not udp then
    udp = socket.udp()
    udp:setpeername(UDP_IP, UDP_PORT)
    log("UDP socket connected to " .. UDP_IP .. ":" .. UDP_PORT)
  end
  udp:send(line_clean .. " \n")
  log(line_clean)
end
...
-----------------------------------------------------------------------
-- Game state helpers (identical to original)
-----------------------------------------------------------------------
...
-----------------------------------------------------------------------
-- Atomic beliefs injection (same as original)
-----------------------------------------------------------------------
function inject_beliefs(g)
    ...
    if my_planet_count == 0 then
        send_to_nars("my_planet_count_zero. :|:")
    elseif my_planet_count == 1 then
        send_to_nars("my_planet_count_one. :|:")
    elseif my_planet_count == 2 then
        send_to_nars("my_planet_count_two. :|:")
    else
        send_to_nars("my_planet_count_three_plus. :|:")
    end

    local enemy_planet_count = #get_enemy_planets(g, g.you)
    if enemy_planet_count == 0 then
        send_to_nars("enemy_count_zero. :|:")
    elseif enemy_planet_count == 1 then
        send_to_nars("enemy_count_one. :|:")
    else
        send_to_nars("enemy_count_two_plus. :|:")
    end
    ...
    if strongest_enemy and strongest_enemy.ships > 20 then
...
function inject_goals(g)
    ...
    -- Correct goal syntax: term! %f;c% :|:
    send_to_nars("advantage! :|:")
    -- send_to_nars(string.format("advantage! %%%.2f;0.9%%", advantage)) -- the problem is herer.
    if not get_strongest_planet(g, g.you) then
        send_to_nars("strong_planet_moderate! :|:")
    end
    send_to_nars("capture_success! :|:")
end

function inject_goals_old(g)
    local my_ships = total_my_ships(g, g.you)
    local enemy_ships = total_enemy_ships(g, g.you)
    local advantage = my_ships / (my_ships + enemy_ships + 0.01)
...
function bot(g)  
  ...
  -- No motor babbling – only execute operations from NARS
...
-----------------------------------------------------------------------
-- Main entry point – does NOT start NARS, only connects to an existing one.
-----------------------------------------------------------------------
function main()
    -- No process management. Just connect to the already running NARS on UDP_PORT.
    log("Waiting for Galcon protocol on stdin. NARS must already be running on " .. UDP_IP .. ":" .. UDP_PORT)
